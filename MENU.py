...
''')
   ...
   def insta(self):
       ...
       self.MyRich(f'''[white]
 >> NAMA         : {self.username} [Keys]
 >> BERGABUNG        : {self.join}
 >> KADARGULA      : {self.expired}
 >> STATUS KAWEN      : {self.status}
 >> USERNAMA         : {self.usr} [Akun]
 >> FULNAMA         : {self.nama}
 >> PENGIKUT        : {self.fol}
 >> USER AIDI          : {self.ds_user} \n''',None,'[white]> [green]DETAIL USER[/] <')

       self.MyRich(''' [white]01. Crack Dari Pengikut
 02. Crack Dari Mengikuti
03. Crack Akun Facebook''',True)
       self.chs(self.aset)
...
\n\tSalin Text Di Atas Untuk Mempercepat!'))
 ...
 02. Crack Dari Komentar        07. Crack Ulang Akun Cepeh
 03. Crack Dari Anggota Group   08. Check Opsi Akun Cepeh
